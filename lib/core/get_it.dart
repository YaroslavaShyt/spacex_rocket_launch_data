...
Future init() async {
  getItInstance.registerLazySingleton<SpaceXRemoteDataSource>(
      () => SpaceXRemoteDataSourceImpl());
  getItInstance.registerLazySingleton<SpaceXRepository>(
      () => SpaceXRepositoryImp(getItInstance()));
  getItInstance.registerLazySingleton<Rockets>(() => Rockets(getItInstance()));
  getItInstance.registerFactory<RocketsBloc>(
          () => RocketsBloc(rockets: getItInstance()));

  getItInstance
      .registerLazySingleton<Missions>(
          () => Missions(getItInstance()));
  getItInstance.registerFactory<MissionsBloc>(
      () => MissionsBloc(missions: getItInstance()));
}
